...
gamma_n_grid = sort(exp(seq(-4, 0.5, length = 20)), decreasing = TRUE)
gamma_p_grid = sort(exp(seq(4.3, 5.3, length = 20)), decreasing = TRUE)
lambda_grid = sort(exp(seq(-10, -6, length = 20)), decreasing = TRUE)

default_gamma_n <- 0.206192
default_gamma_p <- 131.4922
default_lambda <- 0.0002446237

warm_start = funkl2.fit(y1, X1, Ln1, Lp, c(default_gamma_n, default_gamma_p, default_lambda), model = "poisson")

get_loss_at_params <- function(params) {
  
  est = funkl2.fit(y1, X1, Ln1, Lp, params, model = "poisson", theta_init = c(warm_start$alphahat, warm_start$betahat))
...
save.image(file = "nll_plot_l2.RData")
